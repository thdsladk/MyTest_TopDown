...
void AMyTest_TopDownCharacter::PostInitializeComponents()
{
	...
	m_StatComp->OnDeathCheck.AddUObject(this, &AMyTest_TopDownCharacter::Death);

	// Combat Stat			// StatComp의 Init 호출 시점이 플레이어캐릭터의 생성자랑 PostInitalizeComponents 사이라서 
	//						// 현재 위치에 두어야한다. 
	m_AttackRange = m_StatComp->GetTotalStat().AttackRange;
	m_AttackRadius = m_StatComp->GetTotalStat().AttackRadius;
...
void AMyTest_TopDownCharacter::AttackCheck()
{
	...
	if (bResult)
	{
		for (AActor* Result : UniqueResult)
		{
			...
			Result->TakeDamage(m_StatComp->GetAttack(), DamageEvent, GetController(), this);
...
void AMyTest_TopDownCharacter::Click_F()
{
	...
	if (m_CurrentInteractable != nullptr)
	{
		...
		if (Item != nullptr)
		{
			if (ACharacterBase::m_pInventoryComp->TakeItem(*Item) == false)
			{
				// 아이템 얻기 실패
...
void AMyTest_TopDownCharacter::CheckForInteractable()
{
	...
	//FVector StartTrace = m_TopDownCameraComp->GetComponentLocation();
	//FVector EndTrace = (m_TopDownCameraComp->GetForwardVector() * m_Reach) + StartTrace;
...
void AMyTest_TopDownCharacter::HUDUpdate(uint8 Type)
{
	...
	switch (PC->GetHUDState())
	{
		...
		case AMyTest_TopDownPlayerController::EHUDState::EInventory:
		{
			...
			if (Widget != nullptr)
			{
				Widget->BindInventory(GetInventoryComponent_Ref());
				Widget->UpdateWidget();
			}
			break;
		}
...
{
	if (m_StatComp->GetHP() == 0)
		return 0.f;

	OnHit();

	m_StatComp->OnAttacked(DamageAmount);
	return DamageAmount;
}
